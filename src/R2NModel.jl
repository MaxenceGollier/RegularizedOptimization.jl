...
"""
mutable struct R2NModel{T <: Real, V <: AbstractVector{T}, G <: Union{AbstractLinearOperator{T}, AbstractMatrix{T}}, F <: Function} <:
...
function R2NModel(B::G, ∇f::V, σ::T, x0::V) where {T, V, G}
  ...
  v = similar(x0)
  apply_sym = isa(B, AbstractMatrix) ? (B -> Symmetric(B, :L)) : (B -> B)
  return R2NModel(B::G, ∇f::V, v::V, σ::T, apply_sym, meta, Counters())
end

function NLPModels.obj(nlp::R2NModel, x::AbstractVector; skip_sigma = false)
  @lencheck nlp.meta.nvar x
  increment!(nlp, :neval_obj)
  mul!(nlp.v, nlp.apply_sym(nlp.B), x)
